...
2. Store the token and account ID in one of these places, checked in order:
   ...
   - macOS keychain (`security find-generic-password -s harvest -a token|account-id -w`) or Linux `secret-tool lookup service harvest account token|account-id`
...
harvest tasks acme
harvest alias                 # list
harvest alias acme "Acme Website" Development
harvest alias -r acme
harvest whoami
```

Project and task arguments are fuzzy-matched (case-insensitive, `-`/`_`/space equivalent; matches name, code, or client). Ambiguous matches list the candidates. Aliases store resolved IDs, so renames of your alias don't break when project names change.
...
- macOS and Linux (bun + the auth chain above are the only platform dependencies).
